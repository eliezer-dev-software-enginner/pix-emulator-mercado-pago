...
var simulatorRouter = Router();
// ── POST /simulate/pay/:id ────────────────────────────────────────────────────
// Simula o usuário pagando o Pix. Muda status para approved e dispara webhook.
simulatorRouter.post("/pay/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, payment;
        ...
        switch (_a.label) {
            case 0:
                ...
                if (payment.status === "approved") {
                    res.status(409).json({ error: "Pagamento já aprovado", id: id });
                    return [2 /*return*/];
                }
                payment.status = "approved";
                payment.updatedAt = new Date();
                payments.set(id, payment);
                console.log("\n\u2705 Pagamento APROVADO: ".concat(id));
                return [4 /*yield*/, fireWebhook(id, payment.notificationUrl)];
            case 1:
                _a.sent();
...
simulatorRouter.get("/payments", function (req, res) {
    var list = Array.from(payments.values()).map(function (p) { return ({
        id: p.id,
        status: p.status,
        amount: p.amount,
